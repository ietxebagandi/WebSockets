...
export default class CollisionManager{
    ...
    checkCollisions() {
        for (let i = 0; i < this.balls.length; i++) {
            for (let j = 0; j < this.hooks.length; j++) {
                ...
                if ((this.balls[i].position.x - this.balls[i].radius) < (this.hooks[j].position.x) && (this.balls[i].position.x + this.balls[i].radius) > (this.hooks[j].position.x) && ((this.balls[i].position.y - this.balls[i].radius) >= (this.hooks[j].position.y))) {
                    console.log("pang");

                    const newBalls = this.split_ball(this.balls[i]);
    ...
    split_ball(ball){
        ...
        if (ball.radius>Settings.MIN_BALL_RADIUS){
            const ballLeft = new Ball(ball.radius/2, new Vec2D(ball.position.x,ball.position.y), new Vec2D(-ball.force.x,ball.force.y));
            const ballRight = new Ball(ball.radius/2, new Vec2D(ball.position.x,ball.position.y), new Vec2D(ball.force.x,ball.force.y));
            newBalls.push(ballLeft);
            newBalls.push(ballRight)
        }
        return newBalls;
    }
}
